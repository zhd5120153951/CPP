...
namespace DeepSORT_Det {
    ...
    public:
        TrackObjectImpl(const Box &box, 
                    ...
                    const Eigen::Matrix<float, 8, 8> &covariance,
                    int id_next, int nbuckets, int max_age, int nhit, bool has_feature, int clsid, float objconf)
            :nbuckets_(nbuckets), max_age_(max_age), nhit_(nhit), has_feature_(has_feature), clsid(clsid), objconf(objconf)
        {
            last_position_ = box;
		...
		virtual int clsnum() const { return clsid; }
		virtual float score()  const { return objconf; }
        ...
        void update(KalmanFilter &km_filter, const Box &box) {
            ...
            if(trace_.size() > 5){
                const auto center = trace_[trace_.size()-5].center();
                const auto center2 = box.center();
                double res = sqrt(pow(center.x - center2.x, 2) + pow(center.y - center2.y, 2));
                //像素距离大于60，即为有移动，防止原地不动
                if(res>60)
                {
    ...
    class TrackerImpl : public DeespsortTracker
    {
    public:
        ...
        void new_object(const Box &box) {
            ...
            objects_.emplace_back(box, mean, covariance, id_next_, nbuckets_, max_age_, nhit_, has_feature_,box.clsid,box.objconf);
            ++ id_next_;
        }
    ...
    std::shared_ptr<DeespsortTracker> create_tracker(const TrackerConfig& config) {
	...
	void destroy_tracker(std::shared_ptr<DeespsortTracker> _pdeepsort)
	{
        _pdeepsort.reset();
        _pdeepsort = nullptr;
	}

...
namespace SimpleYolo{
    ...
    */
    ...
    public:
        ...
        virtual void set_threshold(float confidence_threshold = 0.3f, float nms_threshold = 0.5f) {
            confidence_threshold_ = confidence_threshold;
            nms_threshold_ = nms_threshold;
        }
    ...
    void image_to_tensor(const cv::Mat& image, shared_ptr<Tensor>& tensor, int ibatch){

        Norm normalize = Norm::alpha_beta(1 / 255.0f, 0.0f, ChannelType::SwapRB);
    ...
    void destroy_infer(shared_ptr<Infer> _pinfer) {
        _pinfer.reset();
        _pinfer = nullptr;
    }


    void set_threshold(std::shared_ptr<Infer> _pinfer, float confidence_threshold, float nms_threshold)
    {
        _pinfer->set_threshold(confidence_threshold, nms_threshold);
    }

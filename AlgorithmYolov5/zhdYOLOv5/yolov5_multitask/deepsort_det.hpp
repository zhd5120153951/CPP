...
struct Box{
    ...
    bool isalarm;
    Box() = default;
    Box(float left, float top, float right, float bottom):left(left), top(top), right(right), bottom(bottom){isalarm = false;}
    Box(float left, float top, float right, float bottom, float objconf, int clsid) :left(left), top(top), right(right), bottom(bottom), objconf(objconf), clsid(clsid) {isalarm = false;}
...
inline Box convert_to_box(const _T& b){
    return Box((float)b.left, (float)b.top, (float)b.right, (float)b.bottom);
}
...
class DeespsortTracker{
public:
    virtual std::vector<TrackObject *> get_objects() = 0;
    virtual void update(const BBoxes& boxes) = 0;
    virtual void track_reset() = 0;
};

std::shared_ptr<DeespsortTracker> create_tracker(

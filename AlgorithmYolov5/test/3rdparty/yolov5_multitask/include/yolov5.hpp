...
namespace SimpleYolo{
    ...
    struct Box{
		...
		cv::Point centerpt() const { return cv::Point((right + left) / 2, (bottom + top) / 2); }
    ...
    public:
        ...
        virtual void set_threshold(float confidence_threshold, float nms_threshold) = 0;
    };
	...
	void set_threshold(std::shared_ptr<Infer> _pinfer, float confidence_threshold = 0.3f,float nms_threshold = 0.5f);
}; // namespace SimpleYolo
